...
- **Schema inventory and coverage yardstick** (`pain001.corpus`,
  ADR-0003 workstream 1). `inventory_for(message_type)` walks a
  bundled XSD into every element path, attribute, `xs:any` slot and
  `xs:choice` group with its branches, with the leaf facets (length,
  pattern, enumeration, digits, bounds) and cardinalities; it
  round-trips through JSON. `coverage(inventory, files, exemptions)`
  reports which paths and branches a set of XML files exercises.
  `make corpus-coverage` (now part of `make check`) measures each
  version's coverage set under `pain001/corpus/data/coverage/<type>/`
  and fails on an incomplete one; versions without a set yet report
  their bundled example for information. The inventories run from
  954 entries and 74 choices (pain.001.001.03) to 1,743 entries and
  133 choices (pain.008.001.08).
- **`docs/message-deltas.md`**, generated by
  `scripts/generate_message_deltas.py` from the inventories: what each
  ISO edition added, removed and changed, .03 through .13 and
  pain.008 .02 to .08, whole blocks folded to one line. It closes the
  research gap on .10 to .13 and corrects a comment in the message
...
- **ISO external code sets, vendored.** Edition 2Q2026 v3 (163 sets,
  ...
  `is_valid`, `describe`, `edition`) and
  `scripts/refresh_external_codes.py`, which swaps in a newer edition
  and refuses when a withdrawn code is still used by bundled data.
