...
def _iter_entry_points(group: str) -> Iterable[metadata.EntryPoint]:
    ...
    if callable(select):
        return cast("Iterable[metadata.EntryPoint]", select(group=group))
    # Legacy importlib.metadata exposed a ``dict`` mapping group -> list.
    return cast(  # pragma: no cover - legacy
        "dict[str, Iterable[metadata.EntryPoint]]", eps
    ).get(group, [])
