...
"""
This module contains the function `create_xml_v10`, which constructs an XML tree
following the ISO 20022 pain.001.001.10 schema.
...
def create_xml_v10(root: et.Element, data: list[dict[str, Any]]) -> et.Element:
    ...
    template = env.get_template("pain.001.001.10/template.xml")
    ...
    # Render the template
    xml_content = template.render(**xml_data_pain001_001_10)

    # Parse the rendered XML content and append its children to the root
    rendered_xml_tree = defused_et.fromstring(xml_content)
