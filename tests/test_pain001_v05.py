...
"""
Comprehensive tests for pain.001.001.05 (ISO 20022 Payment Initiation v05).
...
class TestPain001V5XMLGeneration(unittest.TestCase):
    """Test XML generation for pain.001.001.05 format."""
    ...
    def setUp(self) -> None:
        ...
        csv_path = Path("pain001/templates/pain.001.001.05/template.csv")
        with open(csv_path) as f:
            reader = csv.DictReader(f)
            self.test_data = list(reader)[:2]  # Use first 2 rows

    def test_create_xml_v5_basic(self) -> None:
        """Test basic XML creation for pain.001.001.05."""
        result = create_xml_v5(self.root, self.test_data)
    ...
    def test_create_xml_v5_with_template(self) -> None:
    ...
    def test_create_xml_v5_namespace(self) -> None:
        """Test that correct namespace is used for pain.001.001.05."""
        result = create_xml_v5(self.root, self.test_data)
        xml_string = et.tostring(result, encoding="unicode")

        # Verify pain.001.001.05 namespace is present
        self.assertIn("pain.001.001.05", xml_string)


@pytest.mark.skip(reason="XML example file generation not fully integrated")
class TestPain001V5XSDValidation(unittest.TestCase):
    """Test XSD validation for pain.001.001.05 format."""

    def setUp(self) -> None:
        """Set up test fixtures."""
        self.template_dir = Path("pain001/templates/pain.001.001.05")
        self.xsd_path = self.template_dir / "pain.001.001.05.xsd"
        self.xml_example = self.template_dir / "pain.001.001.05.xml"
    ...
    def test_xml_example_exists(self) -> None:
        ...
        abs_path = (Path(__file__).parent.parent / "pain001" / "templates" /
                   "pain.001.001.05" / "pain.001.001.05.xml")
        self.assertTrue(
...
class TestPain001V5CSVIntegration(unittest.TestCase):
    """Test CSV file integration for pain.001.001.05."""
...
class TestPain001V5DatabaseIntegration(unittest.TestCase):
    """Test SQLite database integration for pain.001.001.05."""
...
class TestPain001V5Templates(unittest.TestCase):
    """Test template files for pain.001.001.05."""
