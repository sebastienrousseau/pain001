...
from pain001.validation import service as service_module
...
def test_data_source_must_be_a_regular_file(tmp_path: Path) -> None:
    ...
    os.mkfifo(pipe)
    result = service_module.ValidationService().validate_data_source(pipe)
...
def test_template_schema_mismatch_is_reported(monkeypatch) -> None:
    ...
    result = service_module.ValidationService().validate_template_schema_compatibility(
        TEMPLATE, SCHEMA
    )
    assert result.is_valid is False
...
def test_data_content_reports_data_source_errors(tmp_path: Path) -> None:
    ...
    empty.write_text("\n", encoding="utf-8")
    result = service_module.ValidationService().validate_data_content(empty)
    assert result.is_valid is False
